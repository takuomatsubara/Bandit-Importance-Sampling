...
def one_experiment_exp(key, log_density, bounds, sample_error):
    #====== set up ======
...
def one_experiment_relu(key, log_density, bounds, sample_error):
    #====== set up ======
    key, _key = jax.random.split(key)
    halton = HaltonSequence(dim=2, bounds=bounds, rngkey=_key)
    bandit = GPBanditSampler(n_init=10, n_eval=2048, sequence=halton)
    ...
    #====== init X ======
    X = halton.generate(jnp.arange(10))
    y = jnp.array([ jnp.exp(log_density(x)) for x in X])
    w = jnp.exp( jnp.log(y) - jsp.special.logsumexp( jnp.log(y) ) )
    sample_loss += [ sample_error.compute(X, w) ]
    #====================
    ...
    for ith in range(10, 100):
        ...
        key, _key = jax.random.split(key)
        gp = fit_gp(_key, X, y)
        objective = jax.vmap(gp.jensen_relu, in_axes=0, out_axes=0)
        #====================

        #====== bandit ======
        X_new = bandit.find(objective)
        y_new = jnp.exp(log_density(X_new))
        #====================
...
def one_experiment_square(key, log_density, bounds, sample_error):
    #====== set up ======
    key, _key = jax.random.split(key)
    halton = HaltonSequence(dim=2, bounds=bounds, rngkey=_key)
    bandit = GPBanditSampler(n_init=10, n_eval=2048, sequence=halton)
    ...
    w = jnp.exp( 2.0*jnp.log(y) - jsp.special.logsumexp( 2.0*jnp.log(y) ) )
    sample_loss += [ sample_error.compute(X, w) ]
    #====================
...
if __name__ == "__main__":
    ...
    if args.experiment == "Exp":
        one_experiment = one_experiment_exp
    elif args.experiment == "Relu":
        one_experiment = one_experiment_relu
    elif args.experiment == "Square":
        one_experiment = one_experiment_square
    else:
    ...
    with open('Data/Phi_{:s}_{:s}_MMD.npy'.format(args.target, args.experiment), 'wb') as f:
