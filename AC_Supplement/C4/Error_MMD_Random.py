...
import sys
sys.path.append("../..")
from src import GP, GPBanditSampler, GPUCB, HaltonSequence, RandomSequence, MMD
...
def one_experiment_bis(key, log_density, bounds, sample_error):
    ...
    random = RandomSequence(2148, dim=2, bounds=bounds, rngkey=_key)
    bandit = GPBanditSampler(n_init=10, n_eval=2048, sequence=random)
    sample_loss = []
    loss_index = jnp.array([12, 15, 19, 24, 31, 39, 49, 62, 78, 99])
    #====================

    #====== init X ======
    X = random.generate(jnp.arange(10))
...
if __name__ == "__main__":
    ...
    if args.target == "Banana":
        ...
        bounds = ( jnp.array([-6.0, -20.0]), jnp.array([6.0, 2.0]) )
